	    				...
	    				success : function(data) {  // 서버와 접속에 성공 했을 때! 
	    					...
	    					if(data.length == 0){ 
	    						...
	    						for(var i in data){
	    							...
	    							// 한 개씩 추가하기!
	    							// 평소에 HTML 태그에 " 를 많이 쓰셨으니까
	    							// ' 로  묶어서 문자열로 표현 할게요!
	    							var innerDiv = '<div class="col-lg-4 col-md-4 col-sm-4 col-xs-6 sideMargin styleOne '
    									+ (data[i].tfilter == 'F' ? 'styleWomen' : data[i].tfilter == 'M' ? 'styleMen' : 'styleAll')
    									+ '" data-aos="fade-right">'
    									...
    									+ '<div class="blog-column">'
    									+ '<ul class="blog-detail list-inline">' 
    									   + '<li><i class="fa fa-user"></i>' + data[i].tcount + '</li>'
    									   + '<li><i class="fa fa-clock-o"></i>'+ data[i].tdate + '</li>'
    									+ '</ul>'
    									+ '<span>' + data[i].ttitle + '</span><br />'
    									+ '<a href="/facehair/selectOne.st?bno=' + data[i].bno + '">Read More</a>'
    								    +'</div>'
    									+'</div>';
    									
    									// 이렇게 해서 c:if를 자바스크립트 형식으로 집어 넣을게요.
    								if(i % 2 == 0) { // i가 처음 시작할 때
    									innerDiv = '<div class="rows">' + innerDiv;
    								} else if(data.length % 2 == 0 && i % 2 != 0) { // 게시글이 짝수이면서 i가 홀수 일 때
    									innerDiv = innerDiv + '</div>';
    								} else { // 게시글이 홀수라서 마지막 i가 혼자 남았을 때 
    									innerDiv = '<div class="rows">' + innerDiv + '</div>';
    								}
	    								$('#styleList').append(innerDiv);
	    							}
	    						
	    						// 이미지 크기 다시 맞추기
	    						$('.images>img').css({'width': '480px', 'height': '350px', 'margin-right': '10px' }); 
	    					}
	    					
	    				}, error : function(){      // 서버와 접속에 실패 했을 때!!
